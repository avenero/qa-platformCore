...
@DisplayName("TableValidationSteps with BrowserEngine")
class TableValidationStepsEngineTest {
    ...
    @Test
    @DisplayName("selección de fila de tabla usa evaluate del engine")
    void seleccionFilaUsaEvaluate() {
        RecordingTableEngine engine = new RecordingTableEngine();
        engine.nextEvaluateResult = "OK";
        activateContext(engine);

        new TableValidationSteps()
            .recorroTablaYSeleccionoLaFilaQueTengaElValorEnLaColumna("#users", "Abel", "Nombre");

        assertThat(engine.evaluateCalls.get()).isEqualTo(1);
        assertThat(engine.lastScript).contains("row.click()");
    }

    @Test
    @DisplayName("validación de cabeceras compara listas normalizadas")
    void validaCabecerasConEvaluate() {
        RecordingTableEngine engine = new RecordingTableEngine();
        engine.nextEvaluateResult = "Id|Nombre|Estado";
        activateContext(engine);

        new TableValidationSteps().validarQueLasCabecerasDeLaTablaSean("#users", "Id, Nombre, Estado");

        assertThat(engine.evaluateCalls.get()).isEqualTo(1);
        assertThat(engine.lastScript).contains("headers.join('|')");
    }
    ...
    private static final class RecordingTableEngine implements BrowserEngine {
        private final AtomicInteger evaluateCalls = new AtomicInteger();
        private Object nextEvaluateResult = null;
