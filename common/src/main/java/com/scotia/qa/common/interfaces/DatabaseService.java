 ...
 *
 * <p>Esta interface unifica tanto <b>operaciones funcionales</b> (queries, CRUD) como
 * <b>gestión técnica</b> (conexiones, pools) en una sola abstracción coherente.
 * Es la interface principal que deben usar todos los frameworks consumidores.
 *
 * <p><b>🎯 RESPONSABILIDADES UNIFICADAS:</b>
 * <ul>
 *   <li><b>Operaciones CRUD</b> - Create, Read, Update, Delete simplificadas</li>
 *   <li><b>Consultas de Testing</b> - Métodos optimizados para validaciones</li>
 *   <li><b>Gestión de Conexiones</b> - Acceso a DataSource y conexiones cuando necesario</li>
 *   <li><b>Operaciones de Negocio</b> - Funcionalidades específicas del dominio</li>
 *   <li><b>Gestión Transaccional</b> - Operaciones multi-statement</li>
 *   <li><b>Utilidades de Validación</b> - Verificaciones de datos para testing</li>
 * </ul>
 *
 ...
 * <ul>
 *   <li>Validar datos en testing de APIs</li>
 *   <li>Preparar data sets para pruebas</li>
 *   <li>Verificar efectos de operaciones</li>
 *   <li>Obtener datos de referencia</li>
 *   <li>Limpiar datos después de tests</li>
 *   <li>Acceso técnico a conexiones para casos especiales</li>
 * </ul>
 *
 ...
 * <ul>
 *   <li><b>BaseDatabaseService:</b> Implementación principal consolidada</li>
 *   <li><b>BaseDatabaseConfiguration:</b> Configuración unificada multi-fuente</li>
 *   <li><b>Extensiones específicas:</b> ApiDatabaseService, WebDatabaseService, etc.</li>
 *   <li><b>Integración automática:</b> Con logging, error handling y pools optimizados</li>
 * </ul>
 *
 * <p><b>📋 NOTA DE CONSOLIDACIÓN:</b>
 * Esta interface unifica lo que anteriormente estaba separado en DatabaseConnector
 * (aspectos técnicos) y DatabaseService (operaciones funcionales). La consolidación
 * simplifica la arquitectura sin perder funcionalidad.
 *
...
public interface DatabaseService extends AutoCloseable {
    ...
    /**
     * Ejecuta una consulta SQL y retorna el primer resultado como Map.
     * Ideal para consultas que retornan una sola fila (ej: buscar usuario por ID).
     *
    ...
    /**
     * Ejecuta una consulta SQL y retorna todos los resultados como lista de Maps.
     * Ideal para consultas que pueden retornar múltiples filas.
     *
     * @param sql consulta SQL a ejecutar
     ...
     * Optimizada para consultas COUNT, SUM, etc.
     *
     ...
     * Más eficiente que queryForCount para verificaciones de existencia.
     *
    ...
    /**
     * Ejecuta múltiples sentencias SQL en una sola transacción (batch).
     ...
     *
     * <p><b>USO LIMITADO:</b> Solo usar cuando necesites acceso directo al pool de conexiones.
     * Para operaciones normales, usar los metodos query/execute de esta interface.
     *
     ...
     *
     * <p><b>IMPORTANTE:</b> La conexión debe ser cerrada explícitamente por el caller
     * usando try-with-resources para evitar leaks de conexión.
     *
     * <p><b>USO LIMITADO:</b> Solo usar cuando necesites JDBC puro para casos muy específicos.
     * Para operaciones normales, usar los metodos query/execute de esta interface.
     *
     ...
     * Incluye version, driver, URL, etc. Útil para debugging.
     *
     ...
     * Incluye queries ejecutadas, tiempo total, errores, etc.
     *
    ...
    /**
     * Obtiene información técnica del pool de conexiones.
     * Útil para monitoring y troubleshooting.
     *
     ...
     * Útil para explorar la estructura de la BD en testing.
     *
     ...
     * Útil para validar estructura de tablas en testing.
     *
     ...
     * Incluye métricas como conexiones activas, tamaño del pool, etc.
     *
    ...
    /**
     * Limpia recursos y cierra conexiones asociadas al servicio.
     * Debe ser llamado al finalizar el uso del servicio.
     */
    void cleanup();
     ...
     * Útil para testing o cuando se necesita limpiar métricas.
     */
    void reset();
    ...
    /**
     * Cierra el servicio y libera todos los recursos asociados.
