...
/**
 * Factory para crear instancias de AuthenticationService del framework Scotia QA.
 *
 * <p>Esta factory proporciona métodos estáticos para crear instancias de AuthenticationService
 * preconfiguradas y listas para usar. Facilita la creación de servicios de autenticación
 * con diferentes configuraciones según las necesidades del framework consumidor.
 *
 * <p><b>Características principales:</b>
 * <ul>
 *   <li>Creación estandarizada de servicios de autenticación</li>
 *   <li>Preconfiguración para diferentes tipos de autenticación</li>
 *   <li>Integración automática con HttpClient</li>
 *   <li>Soporte para múltiples proveedores de autenticación</li>
 ...
 * <ul>
 *   <li><b>OAuth2 Client Credentials</b> - Para autenticación service-to-service</li>
 *   <li><b>Bearer Token</b> - Para autenticación con tokens específicos</li>
 *   <li><b>Basic Auth</b> - Para autenticación usuario/contraseña</li>
 *   <li><b>API Key</b> - Para autenticación con claves de API</li>
 * </ul>
 *
...
public final class AuthenticationServiceFactory {
    ...
    private static final TestLogger.LoggerWrapper log = TestLogger.getLogger(AuthenticationServiceFactory.class);
    ...
    /**
     * Crea una nueva instancia de AuthenticationService con HttpClient automático.
     * Crea internamente un HttpClient usando HttpClientFactory.
     *
    ...
    /**
     * Crea una nueva instancia de AuthenticationService con HttpClient específico.
     * Recomendado cuando ya tienes un HttpClient configurado que quieres reutilizar.
     *
    ...
    /**
     * Crea una instancia preconfigurada para OAuth2 Client Credentials.
     * Configura automáticamente las credenciales del cliente y el endpoint de token.
     *
     ...
     */
    public static AuthenticationService getOAuth2Instance(String clientId, String clientSecret, String tokenEndpoint) {
        ...
        log.debug("AuthenticationService configurado para OAuth2 - clientId: {}, endpoint: {}", clientId, tokenEndpoint);
        return authService;
    }

    /**
     * Crea una instancia preconfigurada para OAuth2 con scopes específicos.
     * Incluye configuración de scopes para control de permisos.
     *
     * @param clientId identificador del cliente OAuth2
     * @param clientSecret secreto del cliente OAuth2
     ...
     * @throws IllegalArgumentException si algún parámetro es null o vacío
     */
    public static AuthenticationService getOAuth2InstanceWithScopes(String clientId, String clientSecret,
    ...
    /**
     * Crea una instancia preconfigurada para Bearer Token.
     * Configura el endpoint de refresh token para bearer tokens.
     *
    ...
    public static AuthenticationService getBearerTokenInstance(String refreshTokenEndpoint) {
        ...
        log.debug("AuthenticationService configurado para Bearer Token - refresh endpoint: {}", refreshTokenEndpoint);
        return authService;
    }

    /**
     * Crea una instancia preconfigurada para Basic Authentication.
     * Nota: La interface actual no tiene método específico para basic auth endpoint.
     * Este método crea una instancia lista para usar con getBasicAuthToken().
     *
    ...
    /**
     * Crea una instancia preconfigurada para API Key authentication.
     * Configura el header donde se enviará la API Key.
     *
    ...
    /**
     * Crea una instancia completamente configurada para un entorno específico.
     * Incluye todas las configuraciones típicas para un ambiente de trabajo.
     *
    ...
    public static AuthenticationService getConfiguredInstance(AuthConfig config) {
        ...
        switch (config.getAuthType().toLowerCase()) {
            ...
            case "apikey":
                ...
                break;
            default:
                throw new IllegalArgumentException("Tipo de autenticación no soportado: " + config.getAuthType());
        }
        ...
        log.debug("AuthenticationService configurado completamente para tipo: {}", config.getAuthType());
        return authService;
    }

    /**
     * Obtiene información de debug sobre las capacidades del factory.
     * Útil para troubleshooting y verificación de configuración.
     *
    ...
    /**
     * Clase de configuración para crear AuthenticationService completamente configurados.
     * Patrón Builder para facilitar la configuración de múltiples parámetros.
     */
    public static class AuthConfig {
        ...
        public String getRefreshTokenEndpoint() { return refreshTokenEndpoint; }
