...
import com.scotia.qa.common.reporting.manager.pipeline.ReportingPipeline;
import com.scotia.qa.common.reporting.manager.pipeline.ReportingStep;
import com.scotia.qa.common.reporting.manager.pipeline.steps.ConversionStep;
import com.scotia.qa.common.reporting.manager.pipeline.steps.ExtentGenerationStep;
import com.scotia.qa.common.reporting.manager.pipeline.steps.JiraUpdateStatusStep;
import com.scotia.qa.common.reporting.manager.pipeline.steps.JiraUploadAttachmentsStep;
...
public class ReportingManager {
    ...
    public static void initialize(ReportingConfig reportingConfig) {
        ...
        TestLogger.logInfo("REPORTING_MANAGER", "   Jira update status: " + config.getJira().isUpdateStatus(), null);
        TestLogger.logInfo("REPORTING_MANAGER", "   Jira upload report: " + config.getJira().isUploadReport(), null);
    }
    ...
    private static ReportingPipeline buildPipeline() {
        ...
        // Step 3: Jira Update Status (si está habilitado)
        steps.add(new JiraUpdateStatusStep());

        // Step 4: Jira Upload Attachments (si está habilitado)
        steps.add(new JiraUploadAttachmentsStep());
