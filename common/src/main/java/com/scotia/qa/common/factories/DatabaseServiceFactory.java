...
import com.scotia.qa.common.implementations.BaseDatabaseService;
import com.scotia.qa.common.logging.TestLogger;

/**
 * Factory para crear instancias de DatabaseService del framework Scotia QA.
 *
 * <p>Esta factory proporciona métodos estáticos para crear instancias de DatabaseService
 * preconfiguradas para diferentes tipos de bases de datos. Simplifica la creación
 * y configuración de servicios de BD para testing y desarrollo.
 *
 * <p><b>Características principales:</b>
 * <ul>
 *   <li>Creación estandarizada de servicios de BD</li>
 *   <li>Soporte multi-base de datos (Oracle, SQL Server, PostgreSQL, MySQL)</li>
 *   <li>Configuraciones predefinidas por tipo de BD</li>
 *   <li>Validación automática de tipos soportados</li>
 *   <li>Logging integrado para troubleshooting</li>
 * </ul>
 *
 * <p><b>Bases de datos soportadas:</b>
 * <ul>
 *   <li><b>Oracle</b> - Usando OracleConnector</li>
 *   <li><b>SQL Server</b> - Usando SqlServerConnector</li>
 *   <li><b>PostgreSQL</b> - Si se configura conector</li>
 *   <li><b>MySQL</b> - Si se configura conector</li>
 * </ul>
 *
 * <p><b>Uso típico:</b>
 * <pre>
 ...
 * <ul>
 *   <li><b>Oracle</b> - Timeout 30s, pool optimizado para consultas</li>
 *   <li><b>SQL Server</b> - Timeout 25s, configuración para transacciones</li>
 *   <li><b>PostgreSQL</b> - Timeout 20s, configuración para JSON</li>
 *   <li><b>MySQL</b> - Timeout 15s, configuración para performance</li>
 * </ul>
 *
...
public final class DatabaseServiceFactory {
    ...
    private static final TestLogger.LoggerWrapper log = TestLogger.getLogger(DatabaseServiceFactory.class);
    ...
    /**
     * Crea una nueva instancia de DatabaseService para el tipo de BD especificado.
     * Utiliza configuraciones optimizadas por defecto según el tipo de base de datos.
     *
    ...
    public static DatabaseService getInstance(String databaseType) {
        ...
        try {
            ...
            throw new RuntimeException("No se pudo crear DatabaseService para tipo: " + normalizedType, e);
        }
    }

    /**
     * Crea una instancia de DatabaseService con timeout personalizado.
     * Útil cuando necesitas configuraciones específicas de timeout.
     *
    ...
    public static DatabaseService getInstanceWithTimeout(String databaseType, int timeoutMs) {
        ...
        String normalizedType = databaseType.toLowerCase().trim();
        log.debug("Creando DatabaseService para {} con timeout personalizado: {} ms", normalizedType, timeoutMs);

        try {
            BaseDatabaseService dbService = new BaseDatabaseService(normalizedType);
            dbService.setConnectionTimeout(timeoutMs);

            log.debug("DatabaseService creado para {} con timeout personalizado: {} ms", normalizedType, timeoutMs);
            return dbService;

        } catch (Exception e) {
            log.error("Error creando DatabaseService para tipo {} con timeout {}: {}",
                normalizedType, timeoutMs, e.getMessage());
            throw new RuntimeException("No se pudo crear DatabaseService para tipo: " + normalizedType, e);
        }
    }

    /**
     * Crea una instancia de DatabaseService específicamente optimizada para Oracle.
     * Incluye configuraciones predefinidas para el mejor rendimiento con Oracle.
     *
    ...
    /**
     * Crea una instancia de DatabaseService específicamente optimizada para SQL Server.
     * Incluye configuraciones predefinidas para el mejor rendimiento con SQL Server.
     *
    ...
    /**
     * Crea una instancia de DatabaseService para PostgreSQL.
     * Configurada para aprovechar características específicas de PostgreSQL.
     *
    ...
    /**
     * Crea una instancia de DatabaseService para MySQL.
     * Configurada para aprovechar características específicas de MySQL.
     *
    ...
    /**
     * Prueba la conectividad con la base de datos especificada.
     * Útil para validar configuraciones antes de usar el servicio.
     *
    ...
    /**
     * Obtiene los tipos de base de datos soportados por el factory.
    ...
    public static String getFactoryInfo() {
        return String.format(
            ...
            BaseDatabaseService.class.getSimpleName(),
            String.join(", ", getSupportedDatabaseTypes())
        );
    }
    ...
    private static void validateDatabaseType(String databaseType) {
        ...
        if (!isSupportedDatabaseType(databaseType)) {
            throw new IllegalArgumentException(
                String.format("Tipo de base de datos no soportado: %s. Tipos soportados: %s",
