 ...
 *
 * <p>Esta clase proporciona toda la funcionalidad HTTP base que pueden usar y extender
 * los frameworks específicos (API, Web, Mobile). Consolida la lógica de peticiones HTTP
 * eliminando duplicaciones y proporcionando una base sólida para cualquier tipo de testing.
 *
 * <p><b>Métodos HTTP soportados:</b>
 * <ul>
 *   <li><b>GET</b> - Para obtención de recursos</li>
 *   <li><b>POST</b> - Para creación de recursos y envío de datos</li>
 *   <li><b>PUT</b> - Para actualización completa de recursos</li>
 *   <li><b>PATCH</b> - Para actualización parcial de recursos</li>
 *   <li><b>DELETE</b> - Para eliminación de recursos</li>
 *   <li><b>HEAD</b> - Para obtener metadatos sin body</li>
 *   <li><b>OPTIONS</b> - Para consultar métodos permitidos</li>
 * </ul>
 *
 * <p><b>Características principales:</b>
 * <ul>
 *   <li>Soporte completo para todos los métodos HTTP estándar (RFC 7231)</li>
 *   <li>Gestión automática de headers, query parameters y body</li>
 *   <li>Configuración flexible de timeouts, redirects y SSL</li>
 *   <li>Logging detallado con sanitización automática de datos sensibles</li>
 *   <li>Cache de respuestas para debugging y análisis de performance</li>
 *   <li>Gestión avanzada de cookies para sesiones cross-request</li>
 *   <li>Contexto de usuario para testing multi-usuario</li>
 *   <li>Validación robusta de parámetros con mensajes descriptivos</li>
 *   <li>Manejo de excepciones tipificadas para debugging eficiente</li>
 *   <li>Thread-safe para uso concurrente en entornos multi-hilo</li>
 * </ul>
 *
 * <p><b>Configuración de contenido:</b>
 * <ul>
 *   <li><b>JSON</b> - {@code configureForJson()} para APIs REST</li>
 *   <li><b>Form Data</b> - {@code configureForFormData()} para formularios web</li>
 *   <li><b>Multipart</b> - {@code configureForMultipart()} para upload de archivos</li>
 *   <li><b>XML</b> - {@code configureForXml()} para servicios SOAP/XML</li>
 *   <li><b>Plain Text</b> - {@code configureForPlainText()} para contenido simple</li>
 * </ul>
 *
 * <p><b>Gestión de timeouts y rendimiento:</b>
 * <ul>
 *   <li>Connection timeout configurable (default: 30 segundos)</li>
 *   <li>Read timeout configurable (default: 60 segundos)</li>
 *   <li>Política de reintentos configurable</li>
 ...
 * </pre>
 *
 * <p><b>Debugging y troubleshooting:</b>
 * La clase proporciona información detallada para debugging:
 * <ul>
 *   <li>Logging automático de requests y responses</li>
 *   <li>Sanitización de datos sensibles (passwords, tokens, etc.)</li>
 *   <li>Información de performance y timeouts</li>
 *   <li>Estado de conexiones y cookies</li>
 *   <li>Contexto de usuario y sesión</li>
 * </ul>
 *
 * <p><b>Nota técnica:</b> Esta implementación utiliza Unirest como cliente HTTP subyacente
 * para proporcionar funcionalidades robustas y confiables. Los frameworks específicos pueden
 * extender o sobrescribir comportamientos según sus necesidades particulares.
 *
...
public class BaseHttpClient implements HttpClient {
     ...
     * Configura automáticamente SSL para deshabilitar validación de certificados en testing.
     */
    ...
    /**
     * Configura SSL para ambientes de testing.
     * Deshabilita la validación de certificados SSL para permitir testing contra
     * servidores con certificados autofirmados o inválidos.
     *
     * <p><b>⚠️ SOLO PARA TESTING:</b> Esta configuración se aplica solo una vez
     * globalmente para todas las instancias de BaseHttpClient.
     */
    ...
    public void addHeader(String key, String value) {
        ...
        if (value == null) {
            ...
            log.debug("Header agregado: {} = {}", normalizedKey, DataUtilities.sanitizeValue(normalizedKey, normalizedValue));
        }
    }
    ...
    @Override
    public HttpResponse executeRequest(HttpMethod method, String endpoint, boolean followRedirects) throws FrameworkTechnicalException {
        if (method == null) {
    ...
    /**
     * Ejecuta una petición HTTP con el método especificado como String.
     * Este método mantiene compatibilidad con el código legacy.
     */
    public HttpResponse executeRequest(String method, String endpoint, boolean followRedirects) throws FrameworkTechnicalException {
        try {
            if (!hasValidHost()) {
                throw new FrameworkTechnicalException("executeRequest", "Host no configurado. Use setHost() primero.");
            }
    ...
    @Override
    public HttpResponse executeRequest(HttpMethod method, String endpoint, int timeoutMs) throws FrameworkTechnicalException {
        if (timeoutMs <= 0) {
    ...
    @Override
    public HttpResponse executeRequest(HttpMethod method, String endpoint, boolean followRedirects, int timeoutMs) throws FrameworkTechnicalException {
        if (timeoutMs <= 0) {
            throw new IllegalArgumentException("Timeout debe ser mayor a 0: " + timeoutMs);
        }
    ...
    @Override
    public void setUserContext(String userId, String sessionId, Map<String, String> additionalContext) {
        ...
        log.debug("Contexto de usuario establecido con {} campos adicionales", additionalContext.size());
    }
    ...
    /**
     * Método protegido que pueden override los frameworks especializados para
     * personalizar la configuración de Unirest.
     */
    ...
    /**
     * Método protegido que pueden override los frameworks especializados para
     * personalizar la construcción de URLs.
     */
    protected String buildFullUrl(String endpoint) throws FrameworkTechnicalException {
        if (currentHost == null) {
            throw new FrameworkTechnicalException("buildFullUrl", "Host no configurado. Use setHost() primero.");
        }
    ...
    /**
     * Método protegido que pueden override los frameworks especializados para
     * personalizar el logging de peticiones.
     */
    ...
    /**
     * Método protegido que pueden override los frameworks especializados para
     * personalizar el logging de respuestas.
     */
    ...
    /**
     * Formatea un string JSON para mostrarlo de forma legible (pretty-print).
     * Si no es JSON válido, devuelve el string original.
     */
    ...
    private kong.unirest.HttpResponse<String> performRequest(String method, String url)
        ...
        switch (httpMethod) {
            ...
            default:
                ...
                throw new FrameworkTechnicalException("performRequest",
                        "Método HTTP no implementado: " + httpMethod +
    ...
    private void ensureContentTypeHeader() {
        ...
        if (!hasContentType) {
            ...
            log.debug("Content-Type ya configurado: {}",
    ...
    private HttpResponse convertToFrameworkResponse(kong.unirest.HttpResponse<String> unirestResponse) {
        HttpResponse response = new HttpResponse(unirestResponse.getStatus(), unirestResponse.getBody());
