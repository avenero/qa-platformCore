...
import com.scotia.qa.common.implementations.BaseDatabaseConfiguration;
...
/**
 * Implementación consolidada del servicio de base de datos para el framework Scotia QA.
 *
 * <p>Esta clase proporciona una implementación completa y robusta del servicio de BD
 * que utiliza directamente la configuración consolidada sin depender del paquete dbConnector.
 * Está optimizada para ser utilizada directamente o extendida por frameworks específicos.
 *
 * <p><b>🔄 CONSOLIDACIÓN ARQUITECTURAL:</b>
 * Esta implementación reemplaza y consolida:
 * <ul>
 *   <li>Funcionalidades de {@code dbConnector/factory/DbConnectorFactory}</li>
 *   <li>Capacidades de {@code dbConnector/connectors/*}</li>
 *   <li>Operaciones de {@code dbConnector/repository/QueryRepository}</li>
 * </ul>
 *
 * <p><b>Características principales:</b>
 * <ul>
 *   <li>Implementación consolidada de DatabaseService</li>
 *   <li>Configuración unificada usando BaseDatabaseConfiguration</li>
 *   <li>Soporte multi-base de datos directo</li>
 *   <li>Gestión automática de conexiones y pools</li>
 *   <li>Logging detallado con sistema TestLogger</li>
 *   <li>Thread-safe para uso concurrente</li>
 *   <li>Operaciones optimizadas para testing</li>
 * </ul>
 *
...
public class BaseDatabaseService implements DatabaseService {
    ...
    private static final TestLogger.LoggerWrapper log = TestLogger.getLogger(BaseDatabaseService.class);
    ...
    public BaseDatabaseService(String databaseType) {
        ...
        try {
            ...
            log.error("Error inicializando BaseDatabaseService para tipo: {} - {}", this.databaseType, e.getMessage());
            throw e;
        }
    }
    ...
    public BaseDatabaseService(String databaseType, String jdbcUrl, String user, String password) {
        ...
        try {
            ...
            this.dataSource = BaseDatabaseConfiguration.createDataSource(this.databaseType, jdbcUrl, user, password);
            log.debug("BaseDatabaseService inicializado con configuración personalizada para tipo: {}", this.databaseType);
        } catch (Exception e) {
    ...
    public Map<String, Object> queryForMap(String sql, Object... parameters) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            throw new RuntimeException("Error ejecutando consulta en " + databaseType + ": " + e.getMessage(), e);
        }
    }
    ...
    public int executeUpdate(String sql, Object... parameters) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            throw new RuntimeException("Error ejecutando update en " + databaseType + ": " + e.getMessage(), e);
        }
    }
    ...
    public int[] executeBatch(List<String> sqlStatements) {
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            throw new RuntimeException("Error ejecutando batch en " + databaseType + ": " + e.getMessage(), e);
        }
    }
    ...
    public int[] executeBatch(String sql, List<Object[]> parametersList) {
        ...
        try (Connection conn = dataSource.getConnection()) {
            log.info("Ejecutando batch parametrizado de {} statements en {}: {}",
                parametersList.size(), databaseType, sanitizeSql(sql));
            ...
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                ...
                log.debug("Batch parametrizado completado exitosamente: {} statements ejecutados", results.length);
                return results;
            ...
            throw new RuntimeException("Error ejecutando batch parametrizado en " + databaseType + ": " + e.getMessage(), e);
        }
    }
    ...
    public Map<String, Object> getDatabaseInfo() {
        try (Connection conn = dataSource.getConnection()) {
            ...
            log.debug("Información de BD obtenida para {}: {}", databaseType, info.get("databaseProductName"));
            return info;
    ...
    public List<Map<String, Object>> getTableColumns(String tableName) {
        ...
        switch (databaseType.toLowerCase()) {
            case "oracle":
                sql = "SELECT column_name, data_type, data_length, nullable FROM user_tab_columns WHERE table_name = UPPER(?) ORDER BY column_id";
                break;
            case "sqlserver":
            case "sql-server":
            case "mssql":
