 ...
 * {@code database-core} puede compartir el mismo id canónico.
 *
 ...
 *
 * <p><b>Nota:</b> a la fecha de creación {@code database-core} no usa {@code @StepDef}
 * (sus steps derivan el id vía {@code @StepId} + nombre de método), por lo que este
 * guard es preventivo: pasa de forma vacía hoy y empezará a vigilar en cuanto el
 * módulo adopte {@code @StepDef}.
 *
 * <p>Usa ArchUnit para leer el bytecode del paquete de steps: nunca inicializa las
 * clases, evitando cargar drivers nativos o dependencias de IO (JDBC).
 *
 * @since W0-S1
 * @see StepDef
 */
class StepCatalogUniquenessTest {
    ...
    private static final String STEPS_PACKAGE = "com.qa.databasecore.steps";
