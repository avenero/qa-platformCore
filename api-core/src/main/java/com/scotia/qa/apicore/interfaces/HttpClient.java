 ...
 *
 * <p>Esta interface proporciona la funcionalidad base que deben implementar todos los clientes HTTP
 * específicos de cada framework (API, Web, Mobile), permitiendo especialización sin duplicación.
 *
 * <p>Características principales: - Configuración de host, headers, query parameters y body -
 * Ejecución de peticiones HTTP con diferentes métodos - Acceso a información de la última petición
 * para debugging - Limpieza de datos de petición - Manejo robusto de excepciones - Validación de
 * parámetros y type safety
 *
 * @author Abel Venero
 * @version 1.0.0
 * @since 2024
...
public interface HttpClient {
  ...
  /**
   * Ejecuta una petición HTTP con el método y endpoint especificados. Método principal type-safe
   * usando enum HttpMethod.
   *
   ...
   * @throws FrameworkTechnicalException Si hay problemas técnicos con la petición (conectividad,
   *     timeout, etc.)
  ...
  /**
   * Ejecuta una petición HTTP con configuración completa per-request. Método más flexible que
   * permite configurar todo sin afectar el cliente global.
   *
   * @param method Método HTTP usando enum type-safe
   * @param endpoint Endpoint relativo
  ...
  /**
   * Limpia todos los datos de configuración de la petición actual (headers, query params, fields,
   * body) pero mantiene el host configurado.
   */
  ...
  /**
   * Establece múltiples cookies para las peticiones HTTP. Crítico para casos cross-framework donde
   * Web captura cookies y API los reutiliza.
   *
  ...
  /**
   * Habilita o deshabilita el manejo automático de cookies. Cuando está habilitado, las cookies de
   * respuesta se almacenan automáticamente.
   *
  ...
  /**
   * Establece el contexto de usuario para las peticiones HTTP. Permite compartir contexto entre
   * frameworks (Web login → API calls → Mobile validation).
   *
   ...
   * @throws IllegalArgumentException Si userId, sessionId son null/vacíos o additionalContext es
   *     null
   */
  ...
  /**
   * Aplica automáticamente el contexto de usuario a los headers de las peticiones. Útil para
   * agregar headers de autorización basados en el contexto.
   *
  ...
  /**
   * Configura automáticamente headers comunes para JSON. Establece Content-Type: application/json y
   * Accept: application/json.
   */
  void configureForJson();

  /**
   * Configura automáticamente headers comunes para XML. Establece Content-Type: application/xml y
   * Accept: application/xml.
   */
  void configureForXml();

  /**
   * Configura automáticamente headers comunes para form data. Establece Content-Type:
   * application/x-www-form-urlencoded.
   */
  void configureForFormData();

  /**
   * Configura automáticamente headers comunes para texto plano. Establece Content-Type: text/plain
   * y Accept: text/plain.
   */
  void configureForPlainText();

  /**
   * Configura automáticamente headers comunes para multipart. Establece Content-Type:
   * multipart/form-data.
   */
  ...
  /**
   * Aplica automáticamente el Content-Type detectado basado en el body actual. No hace nada si no
   * hay body configurado.
   */
  void autoDetectAndSetContentType();

  /**
   * Obtiene una representación string del estado actual del cliente para debugging (sin exponer
   * datos sensibles).
   *
