 ...
 *
 * <ul>
 *   <li>Deshabilitar validación SSL (útil para entornos de desarrollo/testing)
 *   <li>Cargar TrustStores personalizados (.jks files)
 *   <li>Configurar contextos SSL específicos
 *   <li>Gestionar certificados autofirmados
 * </ul>
 *
 * <p><b>⚠️ ADVERTENCIA DE SEGURIDAD:</b> Deshabilitar la validación SSL debe usarse ÚNICAMENTE en
 * entornos controlados de testing. NUNCA usar en producción ya que compromete la seguridad de las
 * comunicaciones.
 *
...
public class SSLUtils {
   ...
   *
   * <p><b>⚠️ SOLO PARA TESTING:</b> Este método deshabilita completamente la validación de
   * certificados SSL, lo cual es un riesgo de seguridad. Usar solo en entornos de desarrollo y
   * testing controlados.
   *
   * <p><b>Casos de uso típicos:</b>
   *
   * <ul>
   *   <li>Testing contra servidores con certificados autofirmados
   *   <li>Ambientes de desarrollo sin certificados válidos
   *   <li>Testing en entornos de QA/UAT con HTTPS
   * </ul>
   *
   ...
   *
   * <p><b>⚠️ SOLO PARA TESTING:</b> Este método deshabilita completamente la validación de
   * hostname, lo cual permite aceptar certificados de cualquier dominio.
   *
   * <p>Debe usarse en conjunto con {@link #createTrustAllSSLContext()} para deshabilitar
   * completamente la validación SSL/TLS.
   *
   ...
   *
   * <p>Este método permite cargar certificados específicos desde un archivo TrustStore (.jks) para
   * validar conexiones SSL de forma segura contra servidores conocidos.
   *
  ...
  public static boolean shouldDisableSSLValidation(String environment) {
    ...
    boolean shouldDisable =
        env.contains("dev") || env.contains("qa") || env.contains("test") || env.contains("local");

    if (shouldDisable) {
      log.warn(
          "⚠️  Ambiente '{}' detectado - Validación SSL puede ser deshabilitada de forma segura",
          environment);
    } else {
